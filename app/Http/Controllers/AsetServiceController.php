...
class AsetServiceController extends Controller
{
    ...
    public function tambahAset(Request $request)
    {
        $request->validate([
            ...
            'masa_pajak' => 'required|date',
            'masa_stnk' => 'required|date',
            'masa_asuransi' => 'required|date',
            'tgl_service_rutin' => 'required|date',
    ...
    public function rekapExport(Request $request)
    {
        ...
        if ($request->kategori == "aset") {
            ...
            $no = 1;
            foreach ($data as $row) {
                fputcsv($handle, array($no, $row->kendaraan->no_polisi, $row->kendaraan->merk, $row->kendaraan->tipe, $row->no_aset, $row->no_polis, $row->no_rangka, $row->no_mesin, Carbon::parse($row->masa_pajak)->translatedFormat('l, d F Y'), Carbon::parse($row->masa_stnk)->translatedFormat('l, d F Y'), Carbon::parse($row->masa_asuransi)->translatedFormat('l, d F Y'), Carbon::parse($row->tgl_service_rutin)->translatedFormat('l, d F Y'), $row->tahun_pembuatan, $row->tahun_pengadaan), ';');
                $no++;
            }
        } else {
            fputcsv($handle, array('Nomor', 'Nomor Polisi', 'Merk', 'Tipe', 'Tanggal Service', 'Kode', 'Uraian'), ';');
            $no = 1;
            foreach ($data as $row) {
                fputcsv($handle, array($no, $row->kendaraan->no_polisi, $row->kendaraan->merk, $row->kendaraan->tipe, Carbon::parse($row->tgl_service)->translatedFormat('l, d F Y'), $row->kode, $row->uraian), ';');
                $no++;
            }
        }
